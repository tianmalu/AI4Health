...
class SupervisedVQVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        return decoded, class_logits, vq_loss, encoding_indices
...
def train_supervised_vqvae(model, train_loader, val_loader, num_epochs=100, lr=1e-4, device='cuda', save_path='best_model.pth', early_stopping_patience = 10):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}")):
            ...
            # Forward pass
            decoded, class_logits, vq_loss, _ = model(data)
        ...
        print(f"  Train Loss: {avg_train_loss:.4f}, Train Acc: {train_acc:.2f}%, Train UAR: {train_uar:.4f}")
        print(f"  Val Loss: {avg_val_loss:.4f}, Val Acc: {val_acc:.2f}%, Val UAR: {val_uar:.4f}")
